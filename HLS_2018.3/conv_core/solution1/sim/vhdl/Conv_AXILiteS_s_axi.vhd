-- ==============================================================
-- File generated on Thu Apr 20 20:44:25 +0800 2023
...
architecture behave of Conv_AXILiteS_s_axi is
    ...
    constant ADDR_CHOUT_V_DATA_0     : INTEGER := 16#28#;
    constant ADDR_CHOUT_V_CTRL       : INTEGER := 16#2c#;
    constant ADDR_KX_V_DATA_0        : INTEGER := 16#30#;
    constant ADDR_KX_V_CTRL          : INTEGER := 16#34#;
    constant ADDR_KY_V_DATA_0        : INTEGER := 16#38#;
    constant ADDR_KY_V_CTRL          : INTEGER := 16#3c#;
    constant ADDR_SX_V_DATA_0        : INTEGER := 16#40#;
    constant ADDR_SX_V_CTRL          : INTEGER := 16#44#;
    constant ADDR_SY_V_DATA_0        : INTEGER := 16#48#;
    constant ADDR_SY_V_CTRL          : INTEGER := 16#4c#;
    constant ADDR_MODE_V_DATA_0      : INTEGER := 16#50#;
    constant ADDR_MODE_V_CTRL        : INTEGER := 16#54#;
    constant ADDR_RELU_EN_V_DATA_0   : INTEGER := 16#58#;
    constant ADDR_RELU_EN_V_CTRL     : INTEGER := 16#5c#;
    constant ADDR_FEATURE_IN_DATA_0  : INTEGER := 16#60#;
    constant ADDR_FEATURE_IN_CTRL    : INTEGER := 16#64#;
    constant ADDR_W_DATA_0           : INTEGER := 16#68#;
    constant ADDR_W_CTRL             : INTEGER := 16#6c#;
    constant ADDR_BIAS_DATA_0        : INTEGER := 16#70#;
    constant ADDR_BIAS_CTRL          : INTEGER := 16#74#;
    constant ADDR_FEATURE_OUT_DATA_0 : INTEGER := 16#78#;
    constant ADDR_FEATURE_OUT_CTRL   : INTEGER := 16#7c#;
    ...
    signal int_CHout_V         : UNSIGNED(15 downto 0) := (others => '0');
    signal int_Kx_V            : UNSIGNED(7 downto 0) := (others => '0');
    signal int_Ky_V            : UNSIGNED(7 downto 0) := (others => '0');
    signal int_Sx_V            : UNSIGNED(7 downto 0) := (others => '0');
    signal int_Sy_V            : UNSIGNED(7 downto 0) := (others => '0');
    signal int_mode_V          : UNSIGNED(0 downto 0) := (others => '0');
    signal int_relu_en_V       : UNSIGNED(0 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CHOUT_V_DATA_0 =>
                        rdata_data <= RESIZE(int_CHout_V(15 downto 0), 32);
                    ...
                    when ADDR_SX_V_DATA_0 =>
                        rdata_data <= RESIZE(int_Sx_V(7 downto 0), 32);
                    when ADDR_SY_V_DATA_0 =>
                        rdata_data <= RESIZE(int_Sy_V(7 downto 0), 32);
                    when ADDR_MODE_V_DATA_0 =>
                        rdata_data <= RESIZE(int_mode_V(0 downto 0), 32);
                    when ADDR_RELU_EN_V_DATA_0 =>
                        rdata_data <= RESIZE(int_relu_en_V(0 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    CHout_V              <= STD_LOGIC_VECTOR(int_CHout_V);
    Kx_V                 <= STD_LOGIC_VECTOR(int_Kx_V);
    Ky_V                 <= STD_LOGIC_VECTOR(int_Ky_V);
    Sx_V                 <= STD_LOGIC_VECTOR(int_Sx_V);
    Sy_V                 <= STD_LOGIC_VECTOR(int_Sy_V);
    mode_V               <= STD_LOGIC_VECTOR(int_mode_V);
    relu_en_V            <= STD_LOGIC_VECTOR(int_relu_en_V);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CHOUT_V_DATA_0) then
                    int_CHout_V(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_CHout_V(15 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SX_V_DATA_0) then
                    int_Sx_V(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_Sx_V(7 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SY_V_DATA_0) then
                    int_Sy_V(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_Sy_V(7 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_MODE_V_DATA_0) then
                    int_mode_V(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_mode_V(0 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_RELU_EN_V_DATA_0) then
                    int_relu_en_V(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_relu_en_V(0 downto 0));
                end if;
            end if;
        end if;
    end process;
