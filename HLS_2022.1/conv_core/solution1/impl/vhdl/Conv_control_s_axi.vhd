...
architecture behave of Conv_control_s_axi is
    ...
    constant ADDR_CHIN_DATA_0        : INTEGER := 16#10#;
    constant ADDR_CHIN_CTRL          : INTEGER := 16#14#;
    constant ADDR_HIN_DATA_0         : INTEGER := 16#18#;
    constant ADDR_HIN_CTRL           : INTEGER := 16#1c#;
    constant ADDR_WIN_DATA_0         : INTEGER := 16#20#;
    constant ADDR_WIN_CTRL           : INTEGER := 16#24#;
    constant ADDR_CHOUT_DATA_0       : INTEGER := 16#28#;
    constant ADDR_CHOUT_CTRL         : INTEGER := 16#2c#;
    constant ADDR_KX_DATA_0          : INTEGER := 16#30#;
    constant ADDR_KX_CTRL            : INTEGER := 16#34#;
    constant ADDR_KY_DATA_0          : INTEGER := 16#38#;
    constant ADDR_KY_CTRL            : INTEGER := 16#3c#;
    constant ADDR_SX_DATA_0          : INTEGER := 16#40#;
    constant ADDR_SX_CTRL            : INTEGER := 16#44#;
    constant ADDR_SY_DATA_0          : INTEGER := 16#48#;
    constant ADDR_SY_CTRL            : INTEGER := 16#4c#;
    constant ADDR_MODE_DATA_0        : INTEGER := 16#50#;
    constant ADDR_MODE_CTRL          : INTEGER := 16#54#;
    constant ADDR_RELU_EN_DATA_0     : INTEGER := 16#58#;
    constant ADDR_RELU_EN_CTRL       : INTEGER := 16#5c#;
    constant ADDR_FEATURE_IN_DATA_0  : INTEGER := 16#60#;
    constant ADDR_FEATURE_IN_DATA_1  : INTEGER := 16#64#;
    constant ADDR_FEATURE_IN_CTRL    : INTEGER := 16#68#;
    constant ADDR_W_DATA_0           : INTEGER := 16#6c#;
    constant ADDR_W_DATA_1           : INTEGER := 16#70#;
    constant ADDR_W_CTRL             : INTEGER := 16#74#;
    constant ADDR_BIAS_DATA_0        : INTEGER := 16#78#;
    constant ADDR_BIAS_DATA_1        : INTEGER := 16#7c#;
    constant ADDR_BIAS_CTRL          : INTEGER := 16#80#;
    constant ADDR_FEATURE_OUT_DATA_0 : INTEGER := 16#84#;
    constant ADDR_FEATURE_OUT_DATA_1 : INTEGER := 16#88#;
    constant ADDR_FEATURE_OUT_CTRL   : INTEGER := 16#8c#;
    ...
    signal int_CHin            : UNSIGNED(15 downto 0) := (others => '0');
    signal int_Hin             : UNSIGNED(15 downto 0) := (others => '0');
    signal int_Win             : UNSIGNED(15 downto 0) := (others => '0');
    signal int_CHout           : UNSIGNED(15 downto 0) := (others => '0');
    signal int_Kx              : UNSIGNED(7 downto 0) := (others => '0');
    signal int_Ky              : UNSIGNED(7 downto 0) := (others => '0');
    signal int_Sx              : UNSIGNED(7 downto 0) := (others => '0');
    signal int_Sy              : UNSIGNED(7 downto 0) := (others => '0');
    signal int_mode            : UNSIGNED(0 downto 0) := (others => '0');
    signal int_relu_en         : UNSIGNED(0 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_CHIN_DATA_0 =>
                        rdata_data <= RESIZE(int_CHin(15 downto 0), 32);
                    when ADDR_HIN_DATA_0 =>
                        rdata_data <= RESIZE(int_Hin(15 downto 0), 32);
                    when ADDR_WIN_DATA_0 =>
                        rdata_data <= RESIZE(int_Win(15 downto 0), 32);
                    when ADDR_CHOUT_DATA_0 =>
                        rdata_data <= RESIZE(int_CHout(15 downto 0), 32);
                    when ADDR_KX_DATA_0 =>
                        rdata_data <= RESIZE(int_Kx(7 downto 0), 32);
                    when ADDR_KY_DATA_0 =>
                        rdata_data <= RESIZE(int_Ky(7 downto 0), 32);
                    when ADDR_SX_DATA_0 =>
                        rdata_data <= RESIZE(int_Sx(7 downto 0), 32);
                    when ADDR_SY_DATA_0 =>
                        rdata_data <= RESIZE(int_Sy(7 downto 0), 32);
                    when ADDR_MODE_DATA_0 =>
                        rdata_data <= RESIZE(int_mode(0 downto 0), 32);
                    when ADDR_RELU_EN_DATA_0 =>
                        rdata_data <= RESIZE(int_relu_en(0 downto 0), 32);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CHIN_DATA_0) then
                    int_CHin(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_CHin(15 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_HIN_DATA_0) then
                    int_Hin(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_Hin(15 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_WIN_DATA_0) then
                    int_Win(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_Win(15 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_CHOUT_DATA_0) then
                    int_CHout(15 downto 0) <= (UNSIGNED(WDATA(15 downto 0)) and wmask(15 downto 0)) or ((not wmask(15 downto 0)) and int_CHout(15 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_KX_DATA_0) then
                    int_Kx(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_Kx(7 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_KY_DATA_0) then
                    int_Ky(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_Ky(7 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SX_DATA_0) then
                    int_Sx(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_Sx(7 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_SY_DATA_0) then
                    int_Sy(7 downto 0) <= (UNSIGNED(WDATA(7 downto 0)) and wmask(7 downto 0)) or ((not wmask(7 downto 0)) and int_Sy(7 downto 0));
                end if;
            end if;
        end if;
    end process;
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_RELU_EN_DATA_0) then
                    int_relu_en(0 downto 0) <= (UNSIGNED(WDATA(0 downto 0)) and wmask(0 downto 0)) or ((not wmask(0 downto 0)) and int_relu_en(0 downto 0));
                end if;
            end if;
        end if;
    end process;
